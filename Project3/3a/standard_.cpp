...
{
    double t_start = 0; double t_end = 5; int n = 1000;
    double t_step = (t_end-t_start)/(n-1);


    vec x = ones<vec>(n); x[0] = 9.008745231347060E-01;         //1.
    vec Vx = ones<vec>(n); Vx[0] = -7.708603521431721E-03*365;  //0;
    vec y = ones<vec>(n); y[0] = 4.354368878037240E-01;         // 0.;
    vec Vy = ones<vec>(n); Vy[0] = 1.546968939253316E-02*365;   // 2*M_PI;
    vec t = ones<vec>(n);
    for(int i = 0; i < n; i++){
        t[i] = t_step*i;
    }
